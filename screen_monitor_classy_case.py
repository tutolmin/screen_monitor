...
#                print("="*50)
        ...
        persist_directory_pres = "./chroma_db_u_pres"
...
#        results = db.similarity_search(test_query, k=min(10, len(all_data['documents'])))
...
Дисциплина 'UX дизайн и конструирование клиентского опыта'
...
#                return retriever_pres.invoke(self.search_query)
...
Ты — эксперт по дисциплине «UX-дизайн и конструирование клиентского опыта». Проанализируй приведённый ниже вопрос и определи его тип:

Knowledge Check — если вопрос направлен на проверку знания факта, определения, термина, этапа процесса или классификации (например: «Что такое CJM?», «Какой метод относится к количественным исследованиям?», «На каком этапе используется Service Blueprint?»). Такой вопрос не содержит описания ситуации и не требует интерпретации данных.
Case Analysis — если вопрос описывает конкретную ситуацию, сценарий взаимодействия клиента, фрагмент исследования, карту путешествия или проблему, и требует анализа, интерпретации, выявления провалов, выбора метода или обоснованного решения на основе UX-подходов.
Ответь строго одним словом:
Knowledge Check
или
Case Analysis

Не объясняй. Не добавляй комментариев. Только одно из двух указанных слов.
